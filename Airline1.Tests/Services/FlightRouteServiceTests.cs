...
namespace Airline1.Tests.Services
{
    public class FlightRouteServiceTests
    {
        ...
        {
            var req = new CreateFlightRouteRequest {
                Code = "R1",
                OriginAirportId = 1,
        ...
        public async Task CreateAsync_Throws_WhenOriginMissing()
        {
            ...
            await Assert.ThrowsAsync<KeyNotFoundException>(() => _service.CreateAsync(req));
        }
        ...
        {
            _mockRepo.Setup(r => r.GetByIdAsync(99)).ReturnsAsync((FlightRoute?)null);
            var req = new UpdateFlightRouteRequest {
                Code = "X",
                OriginAirportId = 1,
                DestinationAirportId = 2,
                DistanceKm = 1000,
            ...
            await Assert.ThrowsAsync<KeyNotFoundException>(() => _service.UpdateAsync(99, req));
        }
        ...
        public async Task UpdateAsync_Throws_WhenNewAirportsInvalid()
        {
            ...
            _mockAirportRepo.Setup(a => a.ExistsAsync(999)).ReturnsAsync(false);
            await Assert.ThrowsAsync<KeyNotFoundException>(() => _service.UpdateAsync(5, req));
        }
